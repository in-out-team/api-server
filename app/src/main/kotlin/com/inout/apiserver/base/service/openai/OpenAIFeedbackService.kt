...
class OpenAIFeedbackService(
    ...
    private val jsonResponseFormat = ChatResponseFormat.JsonObject
    private val textResponseFormat = ChatResponseFormat.Text
...
submittedContent: $submittedContent
    ...
    override fun fetchConversationFeedback(
        fromLanguage: LanguageType,
        toLanguage: LanguageType,
        wordName: String,
        wordMeaning: String,
        conversations: List<FeedbackService.Conversation>,
    ): String {
        val continueConversation =
            conversations.count { it.sender == SenderType.SYSTEM } < RespondToConversationApplication.MAX_SYSTEM_RESPONSES - 1
        val systemDefinitionContent =
            """
...
1. Directly address the user's last conversation first before guiding them toward using the target word.
...
Acknowledge the user's last conversation in a meaningful way, ensuring proper grammar and sentence structure. 
...
${conversations.joinToString(" --> ") { "${it.sender}: ${it.message}" }}
            """.trimIndent()
        val queryMessage =
            ChatMessage(
                role = ChatRole.User,
